...
![产品图](img0/aipro.png){#fig:aipro width=70%}
...
![正面标注视图](img0/1.png){#fig:1 width=100%}

![背面标注视图](img0/2.png){#fig:2 width=100%}

![GPIO接口定义](img0/3.png){#fig:3 width=100%}
...
1. **TF卡**  
   ...
   ![tf卡](img0/tf.jpg){#fig:tf width=30%}

2. **TF卡读卡器**  
   用于在电脑上读写TF卡以刷写系统镜像。建议选择USB 3.0速率的读卡器，以减少系统刷写的等待时间。

   ![读卡器](img0/reader.jpg){#fig:reader width=30%}

3. **HDMI线**  
   开发板配备标准HDMI接口。请根据您的显示器接口类型，准备标准的HDMI线或HDMI转Mini-HDMI/Micro-HDMI线。

   ![HDMI](img0/hdmi.jpg){#fig:hdmi width=30%}

   ![Mini HDMI](img0/minihdmi.jpg){#fig:minihdmi width=30%}

4. **电源适配器**  
   开发板采用PD协议供电（20V挡位），功率需求为65W。请务必使用支持PD协议20V输出的65W Type-C电源适配器。

   ![PD电源](img0/power.png){#fig:power width=30%}
...
14. **树莓派IMX219摄像头（MIPI-CSI）（可选）**  
    ...
    ![MIPI-CSI摄像头](img0/csi.png){#fig:csi width=30%}

15. **MIPI LCD显示屏（可选）**  
    开发板配备MIPI-DSI显示接口，可直接驱动兼容的MIPI显示屏（如树莓派5寸屏），无需外接HDMI显示器。

    ![MIPI显示器](img0/dsi.png){#fig:dsi width=40%}

16. **Micro USB数据线（可选）**  
    开发板板载CH343P芯片，将UART调试串口转换为Micro USB接口。使用Micro USB数据线连接电脑，即可进行串口调试。

    ![Micro USB数据线](img0/microusb.png){#fig:microusb width=30%}
...
![技术支持页面](img0/技术支持.png){#fig:技术支持 width=70%}

在页面下方找到“官方镜像”区域。官方提供了预装昇腾NPU应用环境及软件的Ubuntu和openEuler镜像，极大地方便了开发者快速上手。

![官方镜像](img0/官方镜像.png){#fig:官方镜像 width=70%}
...
1. **点击下载**：点击Ubuntu镜像对应的下载按钮。
   ...
   ![下载](img0/download_ubuntu.png){#fig:download_ubuntu width=30%}

2. **获取提取码**：复制弹出的百度网盘提取码，并点击跳转链接。

   ![跳转](img0/copyandjump.png){#fig:copyandjump width=50%}

3. **选择镜像文件**：在网盘中找到名为`Ubuntu`的文件夹并进入。

   ![文件夹](img0/folder.png){#fig:folder width=100%}
...
5. **选择版本**：
   ...
   **建议初学者下载带有`Desktop`字样的镜像。**
   ...
   ![选择镜像](img0/chooseubuntu.png){#fig:chooseubuntu width=100%}
...
- **Windows**：
  在镜像文件所在文件夹内按住 **Shift** 键并点击鼠标右键，选择“在终端中打开”或“在此处打开Powershell窗口”。输入以下命令（文件名请根据实际情况修改）：
   ...
   ![终端](img0/shell.png){#fig:shell width=50%}  

   ![md5校验](img0/md5.png){#fig:md5 width=100%}
...
2. 打开 **SD Card Formatter** 软件。
   ...
   ![TF卡格式化](img0/SDFmt.png){#fig:SDFmt width=50%}

3. 确认选中了正确的盘符，点击右下角的 **Format** 按钮。

   ![格式化](img0/fmt.png){#fig:fmt width=50%}
   
   *警告：格式化将清除TF卡上所有数据，请确认无误后点击“是”。*

   ![warning](img0/warning.png){#fig:warning width=50%}

4. 等待格式化完成，点击确定。

   ![格式化完成](img0/fmtfin.png){#fig:fmtfin width=50%}

#### 烧录镜像（以Ubuntu为例）

1. 打开 **balenaEtcher**，选择“Flash from file”（从文件烧录）。

   ![balenaEther](img0/ether1.png){#fig:ether1 width=70%}

2. 选择解压后的镜像文件（`.img`格式），然后点击“Select target”选择您的TF卡。

   ![选择磁盘](img0/chooseether.png){#fig:chooseether width=70%}

3. 点击“Flash!”开始烧录。

   ![烧录过程](img0/dd.png){#fig:dd width=50%}

4. 烧录完成后软件会自动进行校验，请耐心等待。

   ![校验过程](img0/val.png){#fig:val width=50%}

5. 校验通过后，关闭软件并安全弹出TF卡。

   ![完毕](img0/finish.png){#fig:finish width=50%}
...
![boot开关](img0/bootswitch.png){#fig:bootswitch width=50%}
...
1. **硬件连接**：
   ...
   ![HDMI0](img0/HDMI0.png){#fig:HDMI0 width=70%}  

   ![TYPE-C Power](img0/typecp.png){#fig:typecp width=70%}

2. **系统启动**：
   - 上电后，风扇会全速旋转，随后声音变小，屏幕显示启动画面。
   - 稍候进入登录界面。

   ![登录](img0/beforelogin.png){#fig:beforelogin width=70%}
