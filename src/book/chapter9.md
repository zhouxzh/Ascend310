...
date: 2026-06-08
subject: "Markdown"
keywords: [AI项目交付, SSD300, 实时检测, 需求拆解, 评测体系, Baseline, 回归]
lang: zh-cn
---

## 9.1 章节总览 {#src-book-chapter9-h1}

本章以“基于 COCO SSD300 的实验室实时检测告警系统”为贯穿案例，建立从需求澄清->指标体系->评测集->迭代节奏->资产沉淀->交付与回归的一套闭环方法论，让技术决策基于指标与风险敞口，而非经验臆测。核心理念：可量化、可比较、可复用、可追溯。

案例默认使用 Hugging Face 仓库 `zhouxzh/SSD300` 中的 `ssd300_resnet50.onnx` / `ssd300_resnet50.om`。其中 `ssd300_` 表示输入尺寸为 300×300，和后续 `ssd320_` SSDLite320 模型区分。实验室告警逻辑只基于 COCO 检测结果做后处理，例如 ROI 闯入、人数统计、背包/电脑/水杯等物品遗留提醒，不重新训练模型，也不额外构建专用数据集。

## 9.2 需求澄清 Canvas {#src-book-chapter9-h2}

| 维度 | 要素 | 问题提示 | 示例 |
| ---- | ---- | -------- | ---- |
| 场景 | 输入源/运行环境 | 摄像头？批处理？ | 实验室入口或设备区摄像头 |
| 目标 | 功能/业务价值 | 用户希望看到什么结果？ | SSD300 实时检测 + 规则告警 |
| 指标 | Latency/FPS/精度 | 哪些分位数重要？ | <80ms / >=25FPS / mAP>=0.6 |
| 约束 | 能耗/内存/带宽 | 上限是多少？ | 功耗<=15W 内存<=3GB |
| 风险 | 数据/硬件/算法 | 失败模式有哪些？ | 低光/遮挡/误报/摄像头抖动 |
| 合规 | 隐私/许可 | 是否需要脱敏？ | 仅上传事件元数据 |
| 成本 | 硬件/云 | ROI 衡量？ | 10 台板卡预算 |

输出：`requirement.yaml`（版本化），后续所有评审基于此文档。

## 9.3 指标分层与优先级 {#src-book-chapter9-h3}
...
## 9.4 Baseline 策略与控制变量法 {#src-book-chapter9-h4}
...
## 9.5 评测集设计原则 {#src-book-chapter9-h5}
...
## 9.6 迭代计划与看板 {#src-book-chapter9-h6}
...
## 9.7 资产沉淀文档体系 {#src-book-chapter9-h7}
...
## 9.8 交付目录与不可变产物 {#src-book-chapter9-h8}
...
## 9.9 上线前综合 Checklist {#src-book-chapter9-h9}
...
## 9.10 验收、回归与漂移监测 {#src-book-chapter9-h10}
