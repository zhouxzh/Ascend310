...
![量化验证代码的职责边界](img8/chapter8_sample_structure.png){#fig:chapter8_sample_structure width=88% .center}

| 职责 | 应该怎么写 | 不应该混进去的内容 |
|---|---|---|
| 路径和默认配置 | 集中定义模型、数据、输出目录，所有程序都从这里取默认值 | 不在每个脚本里重复硬编码路径 |
| 数据读取 | 只负责读取图片清单、解析标签、把图片转成 RGB `uint8` | 不做模型推理，也不计算 accuracy |
| 预处理 | 只负责 resize、HWC->NCHW、归一化、增加 batch 维度 | 不根据 FP32/FP16/INT8 写三套不同逻辑 |
| OM 推理封装 | 只负责 PyACL 上下文、模型加载、输入拷入 NPU、模型执行、输出拷回主机、资源释放 | 不关心 Top1 是否正确 |
| 指标计算 | 只接收 logits 和标签，计算 TopK、输出差异、耗时统计 | 不读取图片，不管理 Ascend 资源 |
| 报告写入 | 把输入条件、模型 hash、指标和少量样本明细写成 JSON/CSV | 不把结论只打印在终端里 |
...
在本章样例中，这些职责分别落到 `chapter8_utils.py`、`acl_resnet_runner.py`、
`01_compare_outputs.py`、`02_perf_compare.py`、`03_prepare_quantization.py`、
`04_calibrate_quantization.py` 和 `05_validate_accuracy.py`。这些文件名不是重点；
重点是读者在写自己的量化验证程序时，也要保留这种“共享输入、共享预处理、
独立指标、结构化报告”的设计。
...
写数据准备代码时，可以用下面几条作为验收标准：

| 验收点 | 为什么重要 |
|---|---|
| 校准图片和验证图片分目录保存 | 避免误把验证集用于校准 |
| 所有图片统一保存为 PNG | 避免 JPEG 有损压缩影响量化对比 |
| 清单使用相对路径 | 开发板和本地目录不同也能迁移 |
| 验证集清单保存标签 | 后续 accuracy 统计不依赖文件名猜类别 |
| 下载程序支持强制刷新和离线 cache | 便于重跑实验，也便于网络不稳定时继续教学 |
...
本章把预处理集中在 `chapter8_utils.py`，核心函数是 `preprocess_resnet_rgb()`：
...
看 Top1、Top5 overlap 和 logits 误差。
...
| 层次 | 实现要点 |
|---|---|
| 输入层 | 读取一批固定图片，通常使用校准图片即可，不需要标签 |
| 推理层 | 同一个 `input_tensor` 先送入 FP32 OM，再送入候选 OM |
| 比较层 | 比较 Top1 是否一致、Top5 交集、最大绝对误差和平均绝对误差 |
