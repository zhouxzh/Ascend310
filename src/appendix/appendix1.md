...
![产品图](img1/aipro.png){#fig:aipro width=70% .center}

### 开发板详细视图 {#src-experiment-case0-h2}

<!-- ![正面视图](img1/4.png){#fig:4 width=70%}

![背面试图](img1/5.png){#fig:5 width=70%} -->

![正面标注视图](img1/1.png){#fig:1 width=100% .center}

![背面标注视图](img1/2.png){#fig:2 width=100% .center}

![GPIO接口定义](img1/3.png){#fig:3 width=70% .center}
...
1. **TF卡**
   ...
   ![tf卡](img1/tf.jpg){#fig:tf width=30% .center}

2. **TF卡读卡器**
   用于在电脑上读写TF卡以刷写系统镜像。建议选择USB 3.0速率的读卡器，以减少系统刷写的等待时间。

   ![读卡器](img1/reader.jpg){#fig:reader width=30% .center}

3. **HDMI线**
   开发板配备标准HDMI接口。请根据您的显示器接口类型，准备标准的HDMI线或HDMI转Mini-HDMI/Micro-HDMI线。

   ![HDMI](img1/hdmi.jpg){#fig:hdmi width=30% .center}

   ![Mini HDMI](img1/minihdmi.jpg){#fig:minihdmi width=30% .center}

4. **电源适配器**
   开发板采用PD协议供电（20V挡位），功率需求为65W。请务必使用支持PD协议20V输出的65W Type-C电源适配器。

   ![PD电源](img1/power.png){#fig:power width=30% .center}
...
6. **金属外壳**
   ...
   ![外壳](img1/cover.png){#fig:cover width=70% .center}

7. **散热风扇及散热鳍片**
   由于昇腾310B处理器在高负载下发热量较大，强烈建议安装主动散热设备。开发板提供2pin风扇接口（12V），支持PWM调速。

   ![风扇](img1/fan.png){#fig:fan width=40% .center}

8. **Type-C转USB 3.0转接线（可选）**
   开发板的一个Type-C接口支持USB 3.0协议（不兼容USB 2.0），可通过转接线连接USB 3.0外设。

   ![转接线](img1/otg.png){#fig:otg width=30% .center}

9. **M.2 NVMe SSD（可选）**
   开发板背部的M.2接口支持PCIe协议（2280规格），可安装NVMe SSD作为系统盘或扩展存储。

   ![nvme ssd](img1/nvme.png){#fig:nvme width=40% .center}

10. **M.2 SATA SSD（可选）**
    该M.2接口同时也支持SATA协议，因此也可以使用M.2 SATA（NGFF）接口的SSD。

    ![ngff ssd](img1/ngff.png){#fig:ngff width=40% .center}

11. **eMMC模块（可选）**
    eMMC是一种高性能、低功耗的嵌入式存储方案。相比TF卡，eMMC读写速度更快（100-400MB/s）且更稳定。开发板预留了eMMC接口，需额外购买香橙派专用eMMC模块。

    ![emmc正面](img1/emmc1.png){#fig:emmc1 width=30% .center}

    ![emmc背面](img1/emmc2.png){#fig:emmc2 width=30% .center}

12. **USB摄像头（可选）**
    用于图像识别、视频通话等应用开发。

    ![摄像头](img1/camera.png){#fig:camera width=40% .center}
...
14. **树莓派IMX219摄像头（MIPI-CSI）（可选）**
    ...
    ![MIPI-CSI摄像头](img1/csi.png){#fig:csi width=30% .center}

15. **MIPI LCD显示屏（可选）**
    开发板配备MIPI-DSI显示接口，可直接驱动兼容的MIPI显示屏（如树莓派5寸屏），无需外接HDMI显示器。

    ![MIPI显示器](img1/dsi.png){#fig:dsi width=40% .center}

16. **Micro USB数据线（可选）**
    开发板板载CH343P芯片，将UART调试串口转换为Micro USB接口。使用Micro USB数据线连接电脑，即可进行串口调试。

    ![Micro USB数据线](img1/microusb.png){#fig:microusb width=30% .center}
...
![技术支持页面](img1/技术支持.png){#fig:技术支持 width=70% .center}

在页面下方找到“官方镜像”区域。官方提供了预装昇腾NPU应用环境及软件的Ubuntu和openEuler镜像，极大地方便了开发者快速上手。

![官方镜像](img1/官方镜像.png){#fig:官方镜像 width=70% .center}
...
1. **点击下载**：点击Ubuntu镜像对应的下载按钮。
   ...
   ![下载](img1/download_ubuntu.png){#fig:download_ubuntu width=30% .center}

2. **获取提取码**：复制弹出的百度网盘提取码，并点击跳转链接。

   ![跳转](img1/copyandjump.png){#fig:copyandjump width=50% .center}

3. **选择镜像文件**：在网盘中找到名为`Ubuntu`的文件夹并进入。

   ![文件夹](img1/folder.png){#fig:folder width=100% .center}
...
5. **选择版本**：
   ...
   **建议初学者下载带有`Desktop`字样的镜像。**
   ...
   ![选择镜像](img1/chooseubuntu.png){#fig:chooseubuntu width=100% .center}
...
1. **点击下载**：点击openEuler镜像对应的下载按钮。
   ...
   ![下载](img1/download_openeuler.png){#fig:download_openeuler width=70%}

2. **获取提取码**：复制提取码并跳转。

   ![跳转](img1/cpjp.png){#fig:cpjp width=70%}

3. **选择镜像文件**：在网盘中进入`OpenEuler`文件夹。

   ![文件夹](img1/folderr.png){#fig:folderr width=70%}

4. **选择版本**：目前官方仅提供带有GUI图形化界面的openEuler镜像。

   ![OpenEuler](img1/chooseeuler.png){#fig:chooseeuler width=70%}
...
- **Windows**：
  在镜像文件所在文件夹内按住 **Shift** 键并点击鼠标右键，选择“在终端中打开”或“在此处打开Powershell窗口”。输入以下命令（文件名请根据实际情况修改）：
   ...
   ![终端](img1/shell.png){#fig:shell width=30% .center}

   ![md5校验](img1/md5.png){#fig:md5 width=100% .center}
...
2. 打开 **SD Card Formatter** 软件。
   ...
   ![TF卡格式化](img1/SDFmt.png){#fig:SDFmt width=50% .center}

3. 确认选中了正确的盘符，点击右下角的 **Format** 按钮。

   ![格式化](img1/fmt.png){#fig:fmt width=50% .center}

   *警告：格式化将清除TF卡上所有数据，请确认无误后点击“是”。*

   ![warning](img1/warning.png){#fig:warning width=50% .center}

4. 等待格式化完成，点击确定。

   ![格式化完成](img1/fmtfin.png){#fig:fmtfin width=50% .center}

#### 烧录镜像（以Ubuntu为例） {#src-experiment-case0-h12}

1. 打开 **balenaEtcher**，选择“Flash from file”（从文件烧录）。

   ![balenaEther](img1/ether1.png){#fig:ether1 width=70% .center}

2. 选择解压后的镜像文件（`.img`格式），然后点击“Select target”选择您的TF卡。

   ![选择磁盘](img1/chooseether.png){#fig:chooseether width=70% .center}

3. 点击“Flash!”开始烧录。

   ![烧录过程](img1/dd.png){#fig:dd width=50% .center}

4. 烧录完成后软件会自动进行校验，请耐心等待。

   ![校验过程](img1/val.png){#fig:val width=50% .center}

5. 校验通过后，关闭软件并安全弹出TF卡。

   ![完毕](img1/finish.png){#fig:finish width=50% .center}
...
![boot开关](img1/bootswitch.png){#fig:bootswitch width=50% .center}
...
1. **硬件连接**：
   ...
   ![HDMI0](img1/HDMI0.png){#fig:HDMI0 width=70% .center}

   ![TYPE-C Power](img1/typecp.png){#fig:typecp width=70% .center}

2. **系统启动**：
   - 上电后，风扇会全速旋转，随后声音变小，屏幕显示启动画面。
   - 稍候进入登录界面。

   ![登录](img1/beforelogin.png){#fig:beforelogin width=70% .center}
...
3. **登录系统**：
   ...
   ![桌面](img1/logingui.png){#fig:desktop width=70% .center}
...
![MicroUSB串口](img1/microusbser.png){#fig:microusbser width=70% .center}

1. **使用Micro USB数据线连接开发板和电脑，此时请不要给开发板上电。**
2. **打开电脑的设备管理器，选择端口，寻找开发板对应的串口端口号**

   ![端口号](img1/ttl.png){#fig:ttl width=70% .center}

3. **打开串口调试软件（PUTTY）**

   ![PUTTY](img1/putty.png){#fig:putty width=70% .center}
...
4. **给开发板上电，等待出现```Ubuntu 22.04.3 LTS orangepiaipro ttyAM0```字样，输入登录的用户名HwHiAiUser并回车，然后输入密码Mind@123并回车，注意在输入密码的时候屏幕并不会显示任何东西，登陆后的界面如图所示。**
   ...
   ![串口](img1/serial.png){#fig:serial width=70% .center}

   ![登录成功](img1/login.png){#fig:login width=70% .center}
...
至此，你的昇腾310B开发板（OrangePi AIpro）已经完成了基本的环境搭建。接下来，你可以进入[案例 1：智能考勤机](../experiment/case1.md)的学习，开始你的第一个AI应用开发之旅。
