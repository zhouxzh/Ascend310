"""
Case 7: Smart Album — Ascend NPU feature extraction with CPU fallback.
...
# ======================================================================
# FeatureExtractor — NPU path + CPU fallback
# ======================================================================

class FeatureExtractor:
    """ResNet50 feature extractor (headless, outputs 2048-dim vectors).
    ...
    def _init_backend(self):
        if os.path.exists(OM_MODEL_PATH):
            try:
                ...
                self.use_npu = True
                print("[FeatureExtractor] Backend: NPU (Ascend 310B)")
                return
            except Exception as exc:
                print(f"[FeatureExtractor] NPU init failed ({exc}), "
        ...
        print("[FeatureExtractor] Backend: CPU (PyTorch ResNet50)")

    def _init_cpu_backend(self):
        import torch
        import torchvision.models as models

        self._torch_model = models.resnet50(
            weights=models.ResNet50_Weights.IMAGENET1K_V1
        )
        # Remove classification head → output 2048-dim feature vector
    ...
    def extract(self, image_bgr):
        """Extract 2048-dim L2-normalized feature vector from a BGR image.

        Returns:
            numpy array shape (2048,) float32, L2-normalized
        """
    ...
    def extract_batch(self, images_bgr_list):
        """Extract features from a list of BGR images.

        On NPU: calls execute() once per image (OM model has fixed batch=1).
        On CPU: stacks images into a single batch for faster inference.

        Returns:
            numpy array shape (N, FEATURE_DIM) float32, L2-normalized
        """
        if not images_bgr_list:
            return np.empty((0, FEATURE_DIM), dtype=np.float32)

        if self.use_npu:
            vectors = [self.extract(img) for img in images_bgr_list]
            return np.stack(vectors, axis=0)
        else:
            import torch

            batch = np.stack(
                [self.preprocess(img) for img in images_bgr_list])
            # Remove singleton batch dim: (N,1,3,224,224) → (N,3,224,224)
