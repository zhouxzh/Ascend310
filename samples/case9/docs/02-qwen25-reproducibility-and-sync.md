...
_定义模型、OM、源码和证据的可重复同步边界；本文件不安装依赖，也不自动启动服务。8T 当前地址为 `192.168.1.90`，`192.168.11.14` 与报告中的 `192.168.8.178` 都是同一块板的历史地址；20T 当前地址为 `192.168.1.95`，`.210` 仅为历史请求地址。_
...
完成 SHA-256 校验，存放于 `reports/board8t/candidate/`。当前 8T 连接入口为
`192.168.1.90`；历史报告中的 `.11.14`/`.178` 地址保持不改写。板端并不存在可供声称的
...
5. 先在 `127.0.0.1:8084` 验证 JSON/SSE，再按 `7868 -> 7867 -> 8084` 验证旧候选链。
