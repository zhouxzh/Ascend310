...
_更新日期：2026-09-05；8T 当前地址是 `192.168.1.90`，历史地址 `.11.14` 和报告采集地址 `.178` 属于同一块板并保留为 provenance。20T 当前地址为 `192.168.1.95`，旧 `.210` 只作历史 provenance；Qwen2.5 当前身份复核为历史 `.90` `passed`（只读）、`.95` `blocked`（当前无工件），详见[缺口账本](28-case9-dual-board-gap-validation-record.md)。IP 变化未触发性能重跑。_
...
监听 `127.0.0.1:8084`。`.178` 是 8T 报告采集地址；板卡历史上使用 `.90` 和 `.11.14`，当前入口为 `.90`，均是同一块板，沿用这些证据，不把地址别名误写成第二块板。

| 板卡 | SoC / 算力 | 环境 | 当前判定 |
| --- | --- | --- | --- |
| `192.168.1.90`（历史 `.11.14`；采集时 `.178`） | Ascend310B4 / 8T | `case9-acl-om`，Python 3.9.25，CANN 8.0.0 | 完整机器门通过；中文工程定性复核单独记录，正式人工签字未完成；候选证据 7 个明确日志/响应文件已本地 SHA-256 校验；当前入口地址 |
