...
class TestNNLayersFloat16:
    ...
    def test_linear_float16(self, in_features, out_features):
        print(f"\nTesting nn.Linear (float16) [in={in_features}, out={out_features}]...")
        # Reduce batch size to avoid OOM on embedded devices
        input_tensor = torch.randn(4, in_features, dtype=torch.float16)
        layer = torch.nn.Linear(in_features, out_features)
        
        # CPU run (float32)
        out_cpu = layer(input_tensor.float())
        
        # NPU run (float16)
        input_npu = input_tensor.npu()
        layer_npu = layer.half().npu()
        out_npu = layer_npu(input_npu)
        
        assert_tensors_close(out_cpu, out_npu, rtol=1e-1, atol=1e-1)
        print("nn.Linear (float16) test passed!")
    ...
    def test_conv2d_float16(self, in_channels, out_channels, kernel_size, stride, padding):
        print(f"\nTesting nn.Conv2d (float16) [in={in_channels}, out={out_channels}, k={kernel_size}, s={stride}, p={padding}]...")
        ...
        h, w = 32, 32 
        input_tensor = torch.randn(1, in_channels, h, w, dtype=torch.float16)
        ...
        print("nn.Conv2d (float16) test passed!")

    @pytest.mark.parametrize("num_features", [64, 128, 256, 512, 1024, 2048])
    def test_batchnorm_float16(self, num_features):
        print(f"\nTesting nn.BatchNorm2d (float16) [C={num_features}]...")
        input_tensor = torch.randn(2, num_features, 14, 14, dtype=torch.float16)
        ...
        assert_tensors_close(out_cpu, out_npu)
        print("nn.BatchNorm2d (float16) test passed!")

    def test_relu_float16(self):
        print("\nTesting nn.ReLU (float16)...")
        ...
        assert_tensors_close(out_cpu, out_npu)
        print("nn.ReLU (float16) test passed!")

    def test_sigmoid_float16(self):
        print("\nTesting nn.Sigmoid (float16)...")
        input_tensor = torch.randn(2, 3, dtype=torch.float16)
        ...
        assert_tensors_close(out_cpu, out_npu)
        print("nn.Sigmoid (float16) test passed!")

    def test_tanh_float16(self):
        print("\nTesting nn.Tanh (float16)...")
        input_tensor = torch.randn(2, 3, dtype=torch.float16)
        layer = torch.nn.Tanh()
        ...
        assert_tensors_close(out_cpu, out_npu)
        print("nn.Tanh (float16) test passed!")

    def test_leaky_relu_float16(self):
        print("\nTesting nn.LeakyReLU (float16)...")
        input_tensor = torch.randn(2, 3, dtype=torch.float16)
        ...
        assert_tensors_close(out_cpu, out_npu)
        print("nn.LeakyReLU (float16) test passed!")

    def test_softmax_float16(self):
        print("\nTesting nn.Softmax (float16)...")
        input_tensor = torch.randn(2, 3, dtype=torch.float16)
        ...
        assert_tensors_close(out_cpu, out_npu)
        print("nn.Softmax (float16) test passed!")

    def test_gelu_float16(self):
        print("\nTesting nn.GELU (float16)...")
        input_tensor = torch.randn(2, 3, dtype=torch.float16)
        layer = torch.nn.GELU()
        ...
        assert_tensors_close(out_cpu, out_npu)
        print("nn.GELU (float16) test passed!")
    ...
    def test_avgpool_float16_default_behavior(self, kernel_size, stride, padding):
        print(f"\nTesting nn.AvgPool2d (float16) [k={kernel_size}, s={stride}, p={padding}]...")
        input_tensor = torch.randn(1, 64, 32, 32, dtype=torch.float16)
        ...
        assert_tensors_close(out_cpu, out_npu)
        print("nn.AvgPool2d (float16) default behavior test passed!")
    ...
    def test_avgpool_float16(self, kernel_size, stride, padding, count_include_pad):
        print(f"\nTesting nn.AvgPool2d (float16) [k={kernel_size}, s={stride}, p={padding}, count_include_pad={count_include_pad}]...")
        input_tensor = torch.randn(1, 64, 32, 32, dtype=torch.float16)
        ...
        assert_tensors_close(out_cpu, out_npu)
        print("nn.AvgPool2d (float16) test passed!")

    @pytest.mark.parametrize("p", [0.5, 0.2])
    def test_dropout_float16(self, p):
        print(f"\nTesting nn.Dropout (float16) [p={p}]...")
        input_tensor = torch.randn(2, 100, dtype=torch.float16)
        ...
        assert_tensors_close(out_cpu, out_npu)
        print("nn.Dropout (float16) test passed!")
    ...
    def test_flatten_float16(self, input_shape):
        print(f"\nTesting nn.Flatten (float16) [input={input_shape}]...")
        input_tensor = torch.randn(*input_shape, dtype=torch.float16)
        ...
        assert_tensors_close(out_cpu, out_npu)
        print("nn.Flatten (float16) test passed!")
    ...
    def test_maxpool_float16_default_behavior(self, kernel_size, stride, padding):
        print(f"\nTesting nn.MaxPool2d (float16) [k={kernel_size}, s={stride}, p={padding}]...")
        input_tensor = torch.randn(1, 64, 112, 112, dtype=torch.float16)
        ...
        assert_tensors_close(out_cpu, out_npu)
        print("nn.MaxPool2d (float16) default behavior test passed!")
    ...
    def test_maxpool_float16(self, kernel_size, stride, padding, ceil_mode):
        print(f"\nTesting nn.MaxPool2d (float16) [k={kernel_size}, s={stride}, p={padding}, ceil_mode={ceil_mode}]...")
        input_tensor = torch.randn(1, 64, 112, 112, dtype=torch.float16)
        # MaxPool2d does not support count_include_pad, using ceil_mode as variant
        ...
        assert_tensors_close(out_cpu, out_npu)
        print("nn.MaxPool2d (float16) test passed!")
