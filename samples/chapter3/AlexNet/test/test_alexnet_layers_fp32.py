...
def create_layer_pair(module):
    """辅助函数：创建 CPU (FP32) 和 NPU (FP32) 版本的层，并同步权重"""
    # 初始化权重
    ...
    # 保持 FP32，不转换为 half()
    npu_module = npu_module.to(device)
    npu_module.eval()
    
    return module, npu_module

def compare_outputs(cpu_out, npu_out, rtol=1e-3, atol=1e-3):
    """辅助函数：对比 CPU (FP32) 和 NPU (FP32) 的输出"""
    npu_out = npu_out.cpu() # 转回 CPU 进行对比，不需要 float() 因为已经是 FP32
