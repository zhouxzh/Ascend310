...
def build_overlay_lines(stats, ffmpeg_proc):
    ...
    return [
        'Scheme: OpenCV BGR -> FFmpeg libx264 -> RTSP',
        ...
        f'PipeWrite: {stats.average("pipe_write_ms"):.1f}ms App->FFmpeg: {stats.pipe_kbps():.1f} kbps {stats.pipe_kb_per_frame():.1f} KB/frame',
    ]
...
ffmpeg_cmd = [
    ...
    '-x264-params', 'bframes=0:sync-lookahead=0:rc-lookahead=0:repeat-headers=1:force-cfr=1',
    '-fflags', 'nobuffer',
    '-flags', 'low_delay',
    '-rtsp_transport', 'tcp',
    '-muxdelay', '0.1',
...
if ffmpeg_proc.poll() is not None:
    ...
    print("当前使用 RTSP 低延迟推流。VLC 播放建议使用 --network-caching=50 到 150。")
...
try:
    while True:
        ...
        try:
            ...
            stats.add_pipe_bytes(len(frame_bytes))
