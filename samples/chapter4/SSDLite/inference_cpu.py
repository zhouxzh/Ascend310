...
import json
# 引入项目中的工具用于解码
from utils_cpu import dboxes320_coco, Encoder, visualize_sample
...
def get_gt_data(item, orig_w, orig_h):
    """
    解析 COCO 格式的 Ground Truth，并缩放到 320x320 的 LTRB 格式，
    ...
    # 计算缩放比例
    scale_x = 320.0 / orig_w
    scale_y = 320.0 / orig_h
...
def visualize_validation_predictions(args, session, encoder, val_dataset):
    ...
    for idx in indices:
        ...
        # 用于可视化的 Tensor (需要去 batch 维度, 3x320x320)
        ...
        # 7. 可视化
        # visualize_sample 会自动把 normalized p_boxes 放大到 320x320，
...
if __name__ == "__main__":
    ...
    parser.add_argument("--backbone", default="mobilenetv2", help="backbone name")
