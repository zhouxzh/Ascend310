...
class Encoder(object):
    """
        ...
        dboxes: default boxes in size 840 x 4,
            encoder: input ltrb format, output xywh format
            decoder: input xywh format, output ltrb format

        encode:
            input  : bboxes_in (Tensor nboxes x 4), labels_in (Tensor nboxes)
            output : bboxes_out (Tensor 840 x 4), labels_out (Tensor 840)
            criteria : IoU threshold of bboexes

        decode:
            input  : bboxes_in (Tensor 840 x 4), scores_in (Tensor 840 x nitems)
    ...
    def decode_single(self, bboxes_in, scores_in, criteria, max_output, max_num=100):
...
def dboxes320_coco(min_ratio=0.1, max_ratio=0.9):
    ...
    # torchvision DefaultBoxGenerator style scale schedule
...
def visualize_sample(img_tensor, gt_boxes, gt_labels, p_boxes, p_labels, p_scores, category_names, save_path):
    ...
    # 3. 画 Prediction (红色)
    # p_boxes 是 normalized [0,1], 需要 scale 到 320
    if p_boxes.numel() > 0:
        p_boxes_np = p_boxes.cpu().numpy() * 320.0 
