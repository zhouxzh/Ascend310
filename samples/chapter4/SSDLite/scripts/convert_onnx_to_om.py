...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Convert SSDLite320 ONNX models to Ascend OM files with atc.")
	parser.add_argument(
		"models",
		nargs="*",
	...
	parser.add_argument(
		...
		help=f"Directory containing ONNX models (default: {WEIGHTS_DIR}).",
	)
	parser.add_argument(
		"--output-dir",
		default="",
		help=f"Directory for generated OM files (default: {MODEL_DIR}).",
	)
	parser.add_argument(
		"--soc-version",
...
def main() -> int:
	...
	output_dir = Path(args.output_dir).expanduser().resolve() if args.output_dir else MODEL_DIR.resolve()
