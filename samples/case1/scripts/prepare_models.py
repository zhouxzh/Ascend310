...
def main():
    ...
    zip_path = MODEL_DIR / 'buffalo_s.zip'
    det_onnx = MODEL_DIR / 'det_500m.onnx'
    if not os.path.exists(det_onnx):
        det_onnx = MODEL_DIR / 'det_10g.onnx'
    rec_onnx = MODEL_DIR / 'w600k_mbf.onnx'
    det_om = MODEL_DIR / 'face_detection.om'
    rec_om = MODEL_DIR / 'face_recognition.om'
    ...
    if need_convert and not args.download_only:
        ...
        if os.path.exists(det_onnx):
            ...
            if not os.path.exists(det_om):
                convert_to_om(det_onnx, str(MODEL_DIR / 'face_detection'), "input.1:1,3,640,640")
            else:
        ...
        if os.path.exists(rec_onnx):
            ...
            if not os.path.exists(rec_om):
                convert_to_om(rec_onnx, str(MODEL_DIR / 'face_recognition'), "input.1:1,3,112,112")
            else:
