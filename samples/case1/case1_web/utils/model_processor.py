...
class ModelProcessor:
    ...
    def load_model(self):
        # 确保context已设置
        if not self.acl_resource.context:
            raise RuntimeError("ACL context is not initialized")
        
        # 设置当前context
        ret = acl.rt.set_context(self.acl_resource.context)
        if ret != 0:
    ...
    def _prepare_input(self, data):
        ...
        ret = acl.rt.set_context(self.acl_resource.context)
        if ret != 0:
            raise RuntimeError(f"acl.rt.set_context failed in _prepare_input: {ret}")
    ...
    def _prepare_output(self):
        ...
        ret = acl.rt.set_context(self.acl_resource.context)
        if ret != 0:
            raise RuntimeError(f"acl.rt.set_context failed in _prepare_output: {ret}")
    ...
    def predict(self, data):
        ...
        try:
            ...
            for i in range(num_outputs):
                ...
                if ret != 0:
                    acl.rt.free_host(host_mem)  # Clean up on error
            ...
            # Always clean up datasets, even if an error occurs
