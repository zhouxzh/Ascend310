"""
Case 5: Motor fault classification — Ascend NPU inference with CPU fallback.

Classifies vibration mel-spectrograms into 5 motor fault types using
EfficientNet-B0. Reuses the AscendResource / AscendModel pattern from
case6/case8.
"""
...
# ======================================================================
# AscendResource / AscendModel — same pattern as case6/case7/case8
# ======================================================================
...
# ======================================================================
# FaultClassifier — NPU path + CPU fallback
# ======================================================================

class FaultClassifier:
    """EfficientNet-B0 motor fault classifier (5 classes).

    Input: mel-spectrogram image (H, W, 3) float32 [0,1]
    Output: fault type + confidence

    Two backends:
      - NPU (Ascend 310B .om model) — requires Cann ACL
      - CPU (PyTorch) — automatic fallback
    """
    ...
    def _init_backend(self):
        if os.path.exists(OM_MODEL_PATH):
            try:
                ...
                self._om_model = AscendModel(self._acl_resource,
                                             OM_MODEL_PATH)
                self.use_npu = True
                print("[FaultClassifier] Backend: NPU (Ascend 310B)")
                return
            except Exception as exc:
                print(f"[FaultClassifier] NPU init failed ({exc}), "
        ...
        print("[FaultClassifier] Backend: CPU (PyTorch EfficientNet-B0)")

    def _init_cpu_backend(self):
        import torch
        import torchvision.models as models

        self._torch_model = models.efficientnet_b0(
            weights=models.EfficientNet_B0_Weights.IMAGENET1K_V1
        )
        in_features = self._torch_model.classifier[1].in_features
        self._torch_model.classifier[1] = torch.nn.Linear(
            in_features, NUM_FAULT_CLASSES
        )
        ...
        if os.path.exists(PTH_MODEL_PATH):
            ...
            print(f"[FaultClassifier] Loaded weights from {PTH_MODEL_PATH}")
        else:
            print("[FaultClassifier] WARNING: no trained weights found — "
    ...
    def preprocess(self, spectrogram_float):
        """Convert mel-spectrogram (H,W,3) float32 [0,1] to NPU tensor.

        Spectrogram is already RGB-like (3-channel replication of
        mel-scale magnitudes). We resize to IMAGE_SIZE and apply
        ImageNet normalization (same pipeline as all other cases).
        """
        import cv2

        # Resize to model input size
        img = cv2.resize(spectrogram_float, (IMAGE_SIZE, IMAGE_SIZE))

        # Already in [0,1] float32, apply ImageNet normalization
    ...
    def classify(self, spectrogram_float):
        """Classify a vibration spectrogram into fault type.

        Args:
            spectrogram_float: (H,W,3) float32 [0,1] mel-spectrogram

        Returns:
            dict with class_id, label_en, label_cn, desc, confidence,
                 all_probs
        """
        input_tensor = self.preprocess(spectrogram_float)
