...
# -*- coding: utf-8 -*-
#
# Vendored from Partitura v1.9.0:
...
#
# Local modification: prepare_notearray accepts this project's structured
# NumPy note array directly, so importing or installing Partitura is not needed.
"""
...
def prepare_notearray(note_info):
    """Return a private pitch/onset/duration/id array for the vendor algorithm."""
    if not isinstance(note_info, np.ndarray) or note_info.dtype.names is None:
        raise TypeError("note_info must be a structured NumPy array")

    required_fields = ("pitch", "onset", "duration")
    for field in required_fields:
        if field not in note_info.dtype.names:
            raise ValueError(
                "Input array does not contain required field {0}".format(field)
            )

    dtypes = dict(note_info.dtype.descr)
    new_dtype = [(name, dtypes[name]) for name in required_fields] + [("id", "i4")]
