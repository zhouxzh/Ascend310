...
> [WebUI 操作、部署与 API](02-webui.md)为准。
...
回退到系统 Python。详细实测数据见 [板端实测结果](10-benchmark-results.md)。
...
双声道。详细证据与命令见 [Ascend 音频输出](06-audio-output.md)。
...
完整逐模型数据和测试边界见 [板端实测结果](10-benchmark-results.md)。
...
模型获取、ATC 与算子验证见 [模型与 OM 部署](03-om-deployment.md)。
...
见 [Upstream 参考仓库](15-upstream-repositories.md)。
