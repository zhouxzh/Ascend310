...
API 与验收。模型准备另见[模型与 OM 部署](03-om-deployment.md)，实测命令和结果另见
[WebUI 触摸屏终审与实机压测](09-webui-acceptance.md)。
...
局域网。输入法和 kiosk 配置见[触摸屏输入法配置](07-touchscreen-input.md)。
...
600 秒 DDSP-VST 双工结果见[WebUI 触摸屏终审与实机压测](09-webui-acceptance.md)。
...
更完整的诊断步骤见[测试故障排查](11-troubleshooting.md)。
