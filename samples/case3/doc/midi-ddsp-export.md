...
当前开发环境若缺少 TensorFlow、DDSP、`pretty_midi` 或 tf2onnx，导出程序会立即报错。
仓库工具不会自动安装这些软件。
...
输出目录默认为 `models/midi_ddsp/stateful_v2/onnx/`，包含 8 个组件：
...
8. Timbre 网络，64 个有效帧和左右各 124 帧 halo。

F0 采样严格保留官方 `top-p=0.95`，休止帧强制为 0 Hz。导出清单同时记录 ONNX
...
python tools/compare_midi_ddsp_stateful_onnx.py \
  --export-manifest models/midi_ddsp/stateful_v2/onnx/export_manifest.json \
...
脚本使用 `Ascend310B4` 和 `mixed_float16` 转换 8 个组件，将 ATC 原始日志保存到
`models/conversion_logs/midi_ddsp_stateful_v2/`，并生成：

```text
models/midi_ddsp/bundles/google-urmp-stateful-v2-mixed_float16/
├── manifest.json
└── midi_ddsp_v2_*.om
```
...
- 旧 32/64 OM 仅在完成 A/B 前保留为 legacy，不与 v2 组件混搭。

当前仓库已经实现上述导出、比较、模型包和运行时代码。本机尚未安装本地导出依赖，
因此新 ONNX/OM 和板端 A/B 结果仍待执行，不能把旧 OM 的实测数据视为 v2 结果。
