...
for (const viewport of viewports) {
  test(`${viewport.name} workspace is stable`, async ({ page }) => {
    ...
    await page.getByRole('tab', { name: '触摸屏' }).click()
...
test('MIDI-DDSP render controls fit a 1366x768 desktop', async ({ page }) => {
  ...
  await expect(page.getByLabel('待渲染 MIDI 概况')).toBeInViewport()
  await expect(page.getByRole('heading', { name: '模型与音色' })).toBeInViewport()
  await expect(page.getByRole('heading', { name: '输出设置' })).toBeInViewport()
  await expect(page.locator('.voice-assignment-row:not(.voice-assignment-header)').first()).toBeInViewport()
...
test('physical 10-inch MIDI-DDSP uses one piano-roll visualizer', async ({ page }) => {
  ...
  await expect(page.getByLabel('待渲染 MIDI 概况')).toBeInViewport()
  await expect(page.getByLabel('MIDI 声部音色分配')).toBeInViewport()
  await expect(page.locator('.voice-assignment-row:not(.voice-assignment-header)').first()).toBeInViewport()
  await expect(page.getByRole('heading', { name: '模型与音色' })).toBeInViewport()
  await expect(page.getByRole('heading', { name: '输出设置' })).toBeInViewport()
  await expect(page.getByTitle('开始渲染')).toBeInViewport()
  await expect(page.locator('.audio-preview, .waveform')).toHaveCount(0)
  await expect(roll).toBeInViewport()
  const noteCanvasIsNonblank = await roll.locator('canvas').nth(1).evaluate((canvas: HTMLCanvasElement) => {
    const context = canvas.getContext('2d')
    if (!context) return false
    return context.getImageData(0, 0, canvas.width, canvas.height).data.some((value, index) => index % 4 === 3 && value > 0)
  })
  expect(noteCanvasIsNonblank).toBe(true)
  const [rollBox, voiceBox, firstVoiceBox, playerBox] = await Promise.all([
    roll.boundingBox(),
    page.getByLabel('MIDI 声部音色分配').boundingBox(),
    page.locator('.voice-assignment-row:not(.voice-assignment-header)').first().boundingBox(),
    page.locator('.midi-player-panel').boundingBox(),
  ])
  expect(rollBox).not.toBeNull()
  expect(voiceBox).not.toBeNull()
  expect(firstVoiceBox).not.toBeNull()
  expect(playerBox).not.toBeNull()
  expect(rollBox!.y + rollBox!.height).toBeLessThanOrEqual(voiceBox!.y + 1)
  expect(firstVoiceBox!.y + firstVoiceBox!.height).toBeLessThanOrEqual(playerBox!.y + playerBox!.height + 1)
  const horizontalOverflow = await page.evaluate(() => document.documentElement.scrollWidth - window.innerWidth)
  expect(horizontalOverflow).toBeLessThanOrEqual(1)
  await page.screenshot({
