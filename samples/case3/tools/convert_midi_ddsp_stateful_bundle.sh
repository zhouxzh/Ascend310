...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/models/conversion_logs/midi_ddsp_stateful_v2_batched}"
...
for row in "${COMPONENTS[@]}"; do
    ...
    if [[ "$RESUME_CONVERSION" == "1" \
        && -s "${output_prefix}.om" \
        && -s "$summary_file" ]] \
        && grep -qx 'ATC_EXIT_CODE=0' "$summary_file" \
        && grep -qx 'ERROR_LINES=none' "$summary_file"; then
    ...
    command=(bash "$SCRIPT_DIR/convert_onnx_to_om.sh" \
        ...
        --log-file "$LOG_DIR/${name}_origin.atc.log" \
