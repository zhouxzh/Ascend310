\section{章节总览}\label{src-book-chapter9-h1}

本章以``基于 COCO SSD300
的实验室实时检测告警系统''为贯穿案例，建立从需求澄清-\textgreater 指标体系-\textgreater 评测集-\textgreater 迭代节奏-\textgreater 资产沉淀-\textgreater 交付与回归的一套闭环方法论，让技术决策基于指标与风险敞口，而非经验臆测。核心理念：可量化、可比较、可复用、可追溯。

案例默认使用 Hugging Face 仓库 \passthrough{\lstinline!zhouxzh/SSD300!}
中的 \passthrough{\lstinline!ssd300\_resnet50.onnx!} /
\passthrough{\lstinline!ssd300\_resnet50.om!}。其中
\passthrough{\lstinline!ssd300\_!} 表示输入尺寸为 300×300，和后续
\passthrough{\lstinline!ssd320\_!} SSDLite320
模型区分。实验室告警逻辑只基于 COCO 检测结果做后处理，例如 ROI
闯入、人数统计、背包/电脑/水杯等物品遗留提醒，不重新训练模型，也不额外构建专用数据集。

\section{需求澄清 Canvas}\label{src-book-chapter9-h2}
...
目标 & 功能/业务价值 & 用户希望看到什么结果？ & SSD300 实时检测 +
规则告警 \\
指标 & Latency/FPS/精度 & 哪些分位数重要？ & \textless80ms /
\textgreater=25FPS / mAP\textgreater=0.6 \\
约束 & 能耗/内存/带宽 & 上限是多少？ & 功耗\textless=15W
内存\textless=3GB \\
...
\section{指标分层与优先级}\label{src-book-chapter9-h3}
...
\section{Baseline 策略与控制变量法}\label{src-book-chapter9-h4}
...
\section{评测集设计原则}\label{src-book-chapter9-h5}
...
\section{迭代计划与看板}\label{src-book-chapter9-h6}
...
看板列：Backlog -\textgreater{} Doing -\textgreater{} Review
-\textgreater{} Bench -\textgreater{} Done；性能/精度任务需进入 Bench
列执行对比脚本通过后才可 Done。

\section{资产沉淀文档体系}\label{src-book-chapter9-h7}
...
\section{交付目录与不可变产物}\label{src-book-chapter9-h8}
...
\section{上线前综合 Checklist}\label{src-book-chapter9-h9}
...
\section{验收、回归与漂移监测}\label{src-book-chapter9-h10}

交付后 7 天加密监控：记录时延、精度漂移（采样对比模型输出变化）。
漂移检测：相同输入集合（Shadow Set）每天抽样跑一次 -\textgreater{} 统计
logits KL 散度/TopK 变化率，高于阈值（如 KL \textgreater{}
0.05）触发报警（潜在数据分布变化或模型文件损坏）。
回归集版本化：\passthrough{\lstinline!eval\_set\_vX!}；若需替换样本
-\textgreater{} 新增版本，不覆盖旧数据。

\section{风险管理与决策日志}\label{src-book-chapter9-h11}
...
\section{章节小结}\label{src-book-chapter9-h12}

方法论的核心不是流程文档堆砌，而是``指标驱动 + 资产沉淀 +
可回滚''三支柱。通过契约化需求、标准化
Baseline、规范化评测与回归体系，使团队协作更高效、风险暴露更透明、交付结果更可信。

\section{实践任务}\label{src-book-chapter9-h13}
