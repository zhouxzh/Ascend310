...
\item
  平均/分位数 (p50/p95)；
\item
  波动系数 CV=std/mean（\textgreater0.15 需进一步剖析）；
\item
...
-\/-\/-\/-\/-\/-\/-\/- \textbar{} \textbar{} Stream Timeline \textbar{}
内核调度顺序 \textbar{} 大量空洞 gap \textbar{} \textbar{} MemCopy
\textbar{} H2D/D2H 开销 \textbar{} 频繁小块拷贝 \textbar{} \textbar{}
Task Kernel \textbar{} 算子执行 \textbar{} 个别算子异常拖长 \textbar{}
\textbar{} Sync/Wait \textbar{} Host 等待 \textbar{} Wait 占比高
\textbar{}
...
\item
  先全量 Profile → 定位热点范围；
\item
  二次局部 Profile（过滤特定算子类型）；
\item
  导出 JSON → 自动解析器归档：算子耗时 TOPK，Copy 占比，Idle 时间。
\end{enumerate}
...
\tightlist
\item
  收集代表性校准集（覆盖光照/尺度/类别分布）；
\item
  校准统计（MinMax / KL）；
\item
  评估 Top1/Top5 差异、关键指标差异（mAP/F1）。 误差定位：Dump
  中间张量（FP32 vs INT8）→ 层级误差分布 →
  定位失真层（常见：激活饱和/尺度不均衡）。
\end{enumerate}
...
\tightlist
\item
  长期 Buffer：模型 I/O、常量 Workspace；
\item
  短期 Buffer：Batch 临时中间；
\item
  建立内存池（按 size class 分类 1KB/4KB/16KB/64KB/大块），分配 → 归还；
\item
  避免频繁
  \passthrough{\lstinline!aclrtMalloc/Free!}：使用池化接口封装；
\item
  监控：每 60s 记录一次池使用率与系统剩余内存，突增后回收未引用对象；
\item
