...
\subsubsection{设置SWAP交换分区}\label{ux8bbeux7f6eswapux4ea4ux6362ux5206ux533a}
...
首先运行\passthrough{\lstinline!sudo swapoff /swapfile!}将原有的swap分区关闭

然后使用\passthrough{\lstinline!sudo fallocate -l 16G /swapfile!}来在系统根目录创建一个swap文件。
\textgreater{}
在创建swap文件的时候，需要注意系统的剩余空间大于所创建文件的大小

修改文件权限\passthrough{\lstinline!sudo chmod 600 /swapfile!}

设置为swap空间\passthrough{\lstinline!sudo mkswap /swapfile!}

启用swap空间\passthrough{\lstinline!sudo swapon /swapfile!}

最后通过\passthrough{\lstinline!free -h!}可以查看swap空间的使用情况。

最后，在/etc/fstab中加入以下命令，在开机时自动挂载swap分区。
\passthrough{\lstinline!/swapfile none swap sw 0 0!}
使用一行命令添加\passthrough{\lstinline!echo '/swapfile none swap sw 0 0' | sudo tee -a /etc/fstab!}
...
\paragraph{安装wiringOP}\label{ux5b89ux88c5wiringop}

在安装wiringOP之前，需要确定系统中存在\passthrough{\lstinline!/etc/orangepi-release!}这个文件，且文件的内容为
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={orangepi-release}]{cases/img0/orangepi-release.png}}
\caption{orangepi-release}
\end{figure}
...
如果提示无法连接，如下图所示
\pandocbounded{\includegraphics[keepaspectratio,alt={无法连接}]{cases/img0/unabletoc.png}}
则尝试以下代码
...
安装完成后使用\passthrough{\lstinline!gpio readall!}验证是否能读取GPIO接口状态，若能正常读取，则说明wiringOP库安装成功，反之请重新尝试安装。

\paragraph{wiringOP-Python}\label{wiringop-python}

这是wiringOP库的Python版本，支持直接在Python中调用 安装过程
...
安装完成后，使用\passthrough{\lstinline!python3 -c "import wiringpi; help(wiringpi)"!}检查该库是否能够正常调用，按q退出。

\subsection{npu-smi相关指令}\label{npu-smiux76f8ux5173ux6307ux4ee4}

\subsubsection{风扇控制}\label{ux98ceux6247ux63a7ux5236}

OrangePi
AiPro的风扇是由pwm进行控制，电源为12V，因此我们可以通过调整pwm的输出频率，来间接调整风扇的转速。

查询风扇的运转模式（PWM模式）
\passthrough{\lstinline!sudo npu-smi info -t pwm-mode!}
输出auto是自动模式，manual则是手动模式。在默认情况下，风扇的运转模式为自动模式，不需要手动调节，如果想要手动调整风扇的转速，需要先将pwm模式修改为手动模式\passthrough{\lstinline!sudo npu-smi set -t pwm-mode -d 0!}

使用\passthrough{\lstinline!sudo npu-smi info -t pwm-duty-ratio!}可以查询风扇的调速比

调整风扇转速\passthrough{\lstinline!sudo npu-smi set -t pwm-duty-ratio -d 100!}，该命令中-d参数后面的数字即是pwm的调速比，可以通过调整调速比来控制风扇的转速。可以输入的数字范围在{[}0-100{]}之间，必须是整数，100是风扇全速运转，0则是风扇停止运转。

\subsubsection{核心控制}\label{ux6838ux5fc3ux63a7ux5236}

开发板使用的泰山CPU共有四个核心，主频均为1.5GHz，在一般情况下，有一个核心会被分给NPU核心，用于cpu与npu之间的通讯，使用\passthrough{\lstinline!sudo npu-smi info -t cpu-num-cfg -i 0 -c 0!}可以查看CPU核心的分配情况。

\passthrough{\lstinline!htop!}命令，类似于Windows系统的任务管理器，可以查看CPU和内存的使用情况，在默认情况下CPU3被用于与npu通信，因此当其他核心被占满时，CPU3的占用率会有显著区别。
如果不想使用Ascend
310B的npu核心，可以使用\passthrough{\lstinline!sudo npu-smi set -t cpu-num-cfg -i 0 -c 0 -v 0:4:0!}将NPU关闭。

\begin{quote}
查询Ai
CPU的占用情况，使用\passthrough{\lstinline!sudo npu-smi info -t usages -i 0 -c 0!}可以查询。
\end{quote}
