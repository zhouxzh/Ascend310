...
\makeatletter
% 先移除旧文件（避免旧的残留）
\immediate\write18{\detokenize{rm -f "output/_includes.tex" 2>/dev/null || del "output\\_includes.tex" 2>nul || true}}
% Linux / macOS (bash)
\immediate\write18{\detokenize{bash -lc "for f in output/*.tex; do [ -f \"\$f\" ] || continue; printf '\\\\input{%s\\\\n' \"\$f\"; done > output/_includes.tex" 2>/dev/null || true}}
% Windows PowerShell
\immediate\write18{\detokenize{powershell -Command "Get-ChildItem -Path output -Filter '*.tex' -File | ForEach-Object { '\\\\input{'+($_.FullName -replace '\\\\','/')+'}' } | Out-File -Encoding utf8 output\\_includes.tex" 2>$null || true}}
\makeatother
