...
% 需要系统已安装相应的 Noto 字体（或在 TeX 系统中可用）
\setmainfont{Noto Serif}[Ligatures=TeX, Scale=MatchLowercase]
\setsansfont{Noto Sans}[Ligatures=TeX, Scale=MatchLowercase]
\setmonofont{Noto Sans Mono}[Scale=MatchLowercase]

\setCJKmainfont{Noto Serif CJK SC}[Scale=1.02]         % 正文字体（衬线 / 宋体样式）
\setCJKsansfont{Noto Sans CJK SC}[Scale=MatchLowercase] % 无衬线（适合章节标题、图例）
\setCJKmonofont{Noto Sans Mono CJK SC}[Scale=MatchLowercase] % 等宽字体（代码、表格）

% 可选：让章节标题使用无衬线以形成层次对比（KOMA-Script）
\addtokomafont{chapter}{\sffamily}
\addtokomafont{section}{\sffamily}

% xeCJK 微调（可选）
\xeCJKsetup{CJKecglue = \lineskip, AutoFakeSlant=false, AutoFakeBold=false}
% 页面布局和外观设置
