...
二者的共性平台基础主要体现在：采用软硬件协同的执行链路（前端表示、编译优化、调度、Runtime）将模型或并行程序映射为底层设备指令序列以提升吞吐与能效；提供涵盖内存管理、算子或 Kernel 调用、性能分析、调试与日志的 API 与工具形成开发与诊断闭环；支持自定义算子或 Kernel 插件的扩展机制以便针对特定负载选择或替换实现；并通过 Profiling、Dump 与分级日志等手段降低性能归因与精度对齐难度。 
