...
昇腾AI异构计算架构CANN（Compute Architecture for Neural Networks）是面向基于达芬奇架构的昇腾处理器的全栈软件体系。上承主流 AI 框架与模型格式，下接硬件与运行时，
覆盖模型转换、编译优化、执行与调试分析的完整链路，发挥承上启下的关键作用，释放昇腾处理器的算力潜能。
