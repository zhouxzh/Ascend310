...
昇腾AI异构计算架构CANN（Compute Architecture for Neural Networks）是面向基于达芬奇架构的昇腾处理器的全栈软件体系。上承主流 AI 框架与模型格式，下接硬件与运行时，覆盖模型转换、编译优化、执行与调试分析的完整链路，发挥承上启下的关键作用，释放昇腾处理器的算力潜能。

异构计算与人工智能相辅相成：AI 模型在训练与推理过程中同时涉及向量/矩阵计算、张量运算、数据搬运与控制流等多类负载，单一处理器难以兼顾性能与能效。异构计算通过专用 AI 核与通用 CPU/GPU/NPU 的协同，使计算与数据流在最合适的单元上执行，从而获得更高吞吐、更低时延与更优能效。
